...
function CreateSessionModal({ classrooms, subjects, onClose }) {
  ...
  const submit = async (event) => {
    ...
    try {
      const payload = { 
        ...form, 
        subject: form.session_type === 'lesson' ? form.subject : null 
      }
      ...
      const nestedFields = data?.error?.details?.fields
      if (nestedFields && nestedFields.length > 0) {
        setError(nestedFields[0])
...
export default function AttendanceDashboard() {
  ...
  useEffect(() => {
    const classroomParams = user?.role === 'teacher' ? { mine: true } : undefined
    Promise.all([
      studentsApi.getClassrooms(classroomParams),
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `attendance_export_${todayISO()}.zip`
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-2">
        <div className="flex gap-2">
          {['today', 'week', 'history'].map(name => (
            <button 
              key={name} 
              onClick={() => setTab(name)} 
              className={`rounded-lg px-4 py-2 text-sm font-medium capitalize ${tab === name ? 'bg-[var(--brand-primary)] text-white' : 'text-gray-600 hover:bg-gray-100'}`}
            >
              {name === 'week' ? 'This Week' : name}
            </button>
          ))}
        </div>
      </Card>
